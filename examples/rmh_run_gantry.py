...
GANTRY_HOME = "/Users/roseh/repositories/robo_mm_olmo_gantry"
...
def main():
    ...
    print("Syncing code")
    rsync_command = f"rsync -rzv --delete --exclude .git --exclude .gitmodules --exclude .idea --exclude '*.html' --exclude '*.pyc' --exclude __pycache__ --exclude .cache --exclude .pytest_cache  --exclude scratch --exclude '*.ipynb' /Users/roseh/repositories/robo_mm_olmo/ {GANTRY_HOME}"
    ...
    gantry_kwargs = dict(
        ...
        gh_token_secret="ROSEH_GITHUB_TOKEN",
        conda=False,
    ...
    env_secret = dict(
        # OPENAI_API_KEY="ROSEH_OPENAI_API_KEY",
        WANDB_API_KEY="ROSEH_WANDB_API_KEY",
        HF_ACCESS_TOKEN="ROSEH_HF_TOKEN",
        # AWS_CREDENTIALS="ROSEH_AWS_CREDENTIALS",
        BEAKER_TOKEN ="ROSEH_BEAKER_TOKEN"
    )
    clusters = []
    ...
    if args.augusta:
        ...
        env.update(
            ...
            MODEL_DIR="/weka/oe-training-default/roseh/mm_olmo/robomolmo_checkpoints/"
        )
        # To allow GCP access,
    ...
    if any(x in command for x in ["train_", "train.py"]):
        ...
        if group and name:
            if args.augusta:
                command += f" --save_folder=gs://oe-training-roseh/molmo-models/{group}/{name}"
            else:
                command += f" --save_folder=/weka/oe-training-default/roseh/mm_olmo/robomolmo_checkpoints/{group}/{name}"
