...
GANTRY_HOME = "/Users/ainaze/Desktop/code/robo_mm_olmo_gantry"
...
def main():
    ...
    print("Syncing code")
    rsync_command = f"rsync -rzv --delete --exclude .git --exclude .gitmodules --exclude .idea --exclude '*.html' --exclude '*.pyc' --exclude __pycache__ --exclude .cache --exclude .pytest_cache  --exclude scratch --exclude '*.ipynb' /Users/ainaze/Desktop/code/robo_mm_olmo/ {GANTRY_HOME}"
    ...
    gantry_kwargs = dict(
        ...
        gh_token_secret="AINAZ_GITHUB_TOKEN",
    ...
    env_secret = dict(
        # OPENAI_API_KEY="AINAZ_OPENAI_API_KEY",
        WANDB_API_KEY="AINAZ_WANDB_API_KEY",
        HF_ACCESS_TOKEN="AINAZ_HF_TOKEN",
        # AWS_CREDENTIALS="AINAZ_AWS_CREDENTIALS",
        BEAKER_TOKEN ="AINAZ_BEAKER_TOKEN"
    )
    clusters = []
    ...
    if args.replicas:
        ...
        if args.augusta:
            ...
            clusters = ["ai2/jupiter"] #["ai2/quarantine-a"]
    else:
        if args.augusta:
            clusters = ["ai2/augusta-google-1"]
        else:
    ...
    if args.augusta:
        ...
        env.update(
            ...
            MODEL_DIR="/weka/oe-training-default/ainaze/mm_olmo/robomolmo_checkpoints/",
            NCCL_DEBUG="INFO",
            NCCL_DEBUG_SUBSYS="ALL",
            TORCH_NCCL_DEBUG="INFO",
            TORCH_NCCL_TRACE_BUFFER_SIZE="1024",
        )
        # To allow GCP access,
    ...
    if any(x in command for x in ["train_", "train.py"]):
        ...
        if group and name:
            if args.augusta:
                command += f" --save_folder=gs://oe-training-ainaze/molmo-models/{group}/{name}"
            else:
                command += f" --save_folder=/weka/oe-training-default/ainaze/mm_olmo/robomolmo_checkpoints/{group}/{name}"
