...
class MolmoConfig(BaseModelConfig):
    ...
    """Point cloud backbone for processing 3D point clouds (optional)"""
...
class Molmo(ModelBase):
    ...
    def __init__(self, config: MolmoConfig, device=None):
        ...
        # Point cloud backbone (optional)
        self.point_cloud_backbone: Optional[PointCloudBackbone] = None
        if self.config.point_cloud_backbone is not None:
            self.point_cloud_backbone = self.config.point_cloud_backbone.build(
                self.config.llm.d_model, device
            )
            # Check if we should freeze the point cloud backbone
            if os.environ.get("FREEZE_POINT_CLOUD_BACKBONE") == "1":
                for param in self.point_cloud_backbone.parameters():
                    param.requires_grad = False
                log.info("Point cloud backbone is FROZEN (requires_grad=False)")
        ...
        # Point cloud token IDs
        self._point_cloud_patch_id = self.special_ids.get(tokenizer.POINT_CLOUD_PATCH_TOKEN)
    ...
    def reset_parameters(self):
        ...
        if self.point_cloud_backbone is not None:
            self.point_cloud_backbone.reset_parameters()
    ...
    def reset_with_pretrained_weights(self):
        ...
        # Point cloud backbone doesn't have pretrained weights by default
    ...
    def apply_activation_checkpointing(self):
        ...
        # Point cloud backbone uses standard checkpointing if needed
    ...
    def forward(
        ...
        point_cloud_masks: Optional[torch.Tensor] = None,
        ...
        # Process point clouds if provided
        if point_clouds is not None and self.point_cloud_backbone is not None:
            point_features, point_valid_mask = self.point_cloud_backbone(
                point_clouds, point_cloud_masks
            )
            # Find point cloud patch positions in input
            is_point_patch = input_ids.view(-1) == self._point_cloud_patch_id
            # Flatten point features for valid patches
            flat_point_features = point_features[point_valid_mask]
            if is_point_patch.sum() > 0 and len(flat_point_features) > 0:
                # Match number of tokens
                num_point_tokens = min(is_point_patch.sum().item(), len(flat_point_features))
                x.view(-1, x.shape[-1])[is_point_patch][:num_point_tokens] += flat_point_features[:num_point_tokens]
