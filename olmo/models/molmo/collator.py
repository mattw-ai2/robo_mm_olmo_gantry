...
class MMCollator:
    ...
    POINT_CLOUD_KEYS = ["point_clouds", "point_cloud_masks"]
    ...
    def __init__(self, special_tokens, max_sequence_length=None, image_padding_lens=None, include_metadata=True, pad=None):
        ...
        # Point cloud special tokens (if available)
        self._point_cloud_tokens = []
        if tokenizer.POINT_CLOUD_START_TOKEN in special_tokens:
            self._point_cloud_tokens = np.array([
                special_tokens[tokenizer.POINT_CLOUD_START_TOKEN],
                special_tokens[tokenizer.POINT_CLOUD_PATCH_TOKEN],
                special_tokens[tokenizer.POINT_CLOUD_END_TOKEN],
            ])[None, :]
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Handle point cloud data if present in any example
        if any("point_clouds" in ex for ex in batch):
            # Find max number of points across batch
            max_points = max(
                (ex["point_clouds"].shape[0] if "point_clouds" in ex and ex["point_clouds"] is not None else 0)
                for ex in batch
            )
            if max_points > 0:
                # Collate point clouds with padding
                point_clouds = []
                point_cloud_masks = []
                for ex in batch:
                    if "point_clouds" in ex and ex["point_clouds"] is not None:
                        pc = ex["point_clouds"]
                        n_points = pc.shape[0]
                        # Pad to max_points
                        padded_pc = np.zeros((max_points, 3), dtype=np.float32)
                        padded_pc[:n_points] = pc[:max_points]
                        point_clouds.append(padded_pc)
                        # Create mask
                        mask = np.zeros(max_points, dtype=bool)
                        mask[:min(n_points, max_points)] = True
                        point_cloud_masks.append(mask)
                    else:
                        # No point cloud for this example
                        point_clouds.append(np.zeros((max_points, 3), dtype=np.float32))
                        point_cloud_masks.append(np.zeros(max_points, dtype=bool))
                
                out["point_clouds"] = torch.from_numpy(np.stack(point_clouds))
                out["point_cloud_masks"] = torch.from_numpy(np.stack(point_cloud_masks))
